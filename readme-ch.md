...
- 一键检查 `dsh`、Node.js,以及插件需要写入的三个路径
- 每项失败都给出**具体怎么办**:安装命令、路径该填什么,或常见原因(vault 在 iCloud / OneDrive / Dropbox 同步目录里、目录只读、权限不足)
- 排查不下来可以点 **「复制诊断信息」**:生成一段自包含的说明,**预览后**再复制,粘贴给任意 AI 助手或 DSH 桌面端帮你诊断

### 🔄 更新检查

- 设置页可检查是否有新版本,并告诉你去哪里更新
- 插件**不会自己下载或安装更新**:更新由 Obsidian 自己完成,不会绕开商店的审核与签名校验
...
| 模型列表 | 内置 4 个 | 下拉框里可选的模型,可任意增删;「从 DSH 导入」按一下才读取你自己 DSH 配置里声明的模型 ID |
...
- 插件不收集任何遥测数据；除「检查更新」在你手动点击时访问 GitHub 的 Release 接口外，自身不发起网络请求。
- 凭据默认全部走 DSH 既有配置，可选插件专属 Key 只存在本机插件数据文件，运行时以环境变量注入；诊断报告不含任何凭据。
